 ...
 *
 * This interface enables strong typing of the getFee method by mapping
 * each operation type to its specific parameter structure, ensuring
 * compile-time validation of parameter compatibility.
 *
 * @example
 * ```typescript
...
interface AppKitContext {
     ...
     *
     * Calculate the estimated fees for cross-chain bridge transfers or same-chain
    ...
    /**
     * Event handlers registered for various operations in AppKit.
     *
     * This property stores event handlers that are registered via the AppKit's
     * `on()` method. Handlers are organized by operation type (e.g., 'bridge') and
     * keyed by their action name (prefixed with the operation type, e.g., 'bridge.approve',
     * or '*' for wildcard handlers). Each action can have multiple handlers registered,
     * allowing multiple subscribers to listen to the same event.
     *
     * The handlers are stored in the context to allow deferred registration with
     * underlying kit instances (BridgeKit, SwapKit, etc.), enabling a clean separation
     * between event registration and operation execution. This design supports extensibility
     * for future operation types that may be added to AppKit.
     *
     * @example
     * ```typescript
     * const context = createContext()
     * // Handlers are automatically stored here when kit.on() is called
     * // Bridge handlers are registered with BridgeKit when bridge() is executed
     * // Future operation handlers (e.g., swap, stake) will be registered similarly
     * ```
     */
...
export type { ActionHandler, AdapterContext, AllowanceStrategy$2 as AllowanceStrategy, AppKitActionName, AppKitActions, AppKitBridgeActions, AppKitConfig, AppKitContext, AppKitUnifiedBalanceActions, BaseChainDefinition, BridgeConfig, CustomFeePolicy$2 as BridgeCustomFeePolicy, BridgeParams, BridgeResult, BridgeStep, CCTPConfig, CCTPMergedConfig, CCTPSplitConfig, ChainDefinition, Currency, DelegateStatus, DepositForParams, DepositParams, DepositResult, DeveloperFeeHooks, EVMChainDefinition, ErrorDetails, EstimateResult$1 as EstimateResult, EstimateSpendResult, EstimatedGas, FeeOperationType, GetBalancesParams, GetBalancesResult, GetDelegateStatusParams, GetSupportedChainsOptions, InitiateRemoveFundParams, InitiateRemoveFundResult, KitContractType, NonEVMChainDefinition, OperationType, Recoverability, RemoveFundParams, RemoveFundResult, RetryContext, SendParams, SpendDestination, SpendParams, SpendResult, SwapConfig, CustomFeePolicy$1 as SwapCustomFeePolicy, SwapEstimate, SwapFeeContext, SwapKitConfig, SwapKitContext, SwapParams, SwapResult, TokenInfo, UnifiedBalanceChainIdentifier, CustomFeePolicy as UnifiedBalanceCustomFeePolicy, UnifiedBalanceKitConfig, SupportedToken as UnifiedBalanceSupportedToken, UpdateDelegateParams, UpdateDelegateResult, VersionConfig };
